...
<body class="h-100 bg-dark">
    <div class="container-fluid d-flex align-items-center justify-content-center">
        <div class="content bg-secondary">
            ...
            <form method="post" action="M_aluno.php" id="apagarAluno" name="apagarAluno">
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        $('#saveBtn').on('click', async function(e) {
            ...
            const request = await fetch(base_url + 'Aluno/apagarAluno', config);
            ...
            if (response.codigo == 6) {
                ...
                Swal.fire({
                    title: response.codigo + ' - ' + 'Aluno desativado corretamente',
                    icon: 'success'
                });
            } else if (response.codigo == 2) {
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'Houve algum problema na desativação do aluno',
                    icon: 'warning'
                });
            } else if (response.codigo == 3) {
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'Aluno já desativado',
                    icon: 'warning'
                });
            } else if (response.codigo == 4) {
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'O RA informado não está cadastrado na base de dados',
                    icon: 'warning'
                });
