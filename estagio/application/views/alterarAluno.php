...
<body class="h-100 bg-dark">
    <div class="container-fluid d-flex align-items-center justify-content-center">
        <div class="content bg-secondary">
            ...
            <form method="post" action="M_aluno.php" id="alterarAluno" name="alterarAluno">
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        $('#saveBtn').on('click', async function(e) {
            ...
            const request = await fetch(base_url + 'Aluno/alterarAluno', config);
            ...
            if (response.codigo == 6) {
                ...
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'Informe um dado para alteração',
                    icon: 'warning'
                });
            } else if (response.codigo == 8) {
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'ID do curso não pode ser zerado',
                    icon: 'warning'
                });
            } else if (response.codigo == 1) {
                Swal.fire({
                    title: response.codigo + ' - ' + 'Dados do aluno atualizados corretamente',
                    icon: 'success'
                });
            } else if (response.codigo == 2) {
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'Houve algum problema na atualização do aluno',
                    icon: 'warning'
                });
            } else if (response.codigo == 3) {
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'Não é possível alterar, pois o aluno está desativado',
                    icon: 'warning'
                });
                ...
                Swal.fire({
                    title: 'Atenção!',
                    text: response.codigo + ' - ' + 'Curso não localizado',
                    icon: 'warning'
                });
            } else {
                Swal.fire({
